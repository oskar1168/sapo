...
let CURRENCY_CONVERSION_RATE = 9.0; // 100 JPY = 900 KRW (실시간 환율 동적 갱신용)
...
let translatorSourceLang = "ko";
let translatorTargetLang = "ja";
...
const elements = {
  ...
  // Trip Header Edit Modal (Title & Subtitle)
  modalHeaderEdit: document.getElementById("modalHeaderEdit"),
  btnHeaderEditClose: document.getElementById("btnHeaderEditClose"),
  btnHeaderEditCancel: document.getElementById("btnHeaderEditCancel"),
  formHeaderEdit: document.getElementById("formHeaderEdit"),
  inputHeaderTitle: document.getElementById("inputHeaderTitle"),
  inputHeaderSubtitle: document.getElementById("inputHeaderSubtitle"),

  // HOT Guide Modal
  modalExploreGuide: document.getElementById("modalExploreGuide"),
  btnGuideModalClose: document.getElementById("btnGuideModalClose"),
  btnGuideModalCancel: document.getElementById("btnGuideModalCancel"),
  guideModalBody: document.getElementById("guideModalBody"),

  // Realtime Weather & Tools Elements
  wrapperTools: document.getElementById("wrapperTools"),
  weatherIcon: document.getElementById("weatherIcon"),
  weatherLabel: document.getElementById("weatherLabel"),
  txtWeatherValue: document.getElementById("txtWeatherValue"),
  forecastContainer: document.getElementById("forecastContainer"),
  weatherGuideText: document.getElementById("weatherGuideText"),

  // Realtime Exchange Rate Elements
  txtExchangeRateValue: document.getElementById("txtExchangeRateValue"),
  txtExchangeRateTime: document.getElementById("txtExchangeRateTime"),

  // Liked Spots Slider Elements
  likedSpotsSection: document.getElementById("likedSpotsSection"),
  likedSpotsSlider: document.getElementById("likedSpotsSlider"),

  // Place Spot Info Modal Elements (일정 등록 시 추천 스팟용)
  placeSpotInfoWrapper: document.getElementById("placeSpotInfoWrapper"),
  lblPlaceSpotMenu: document.getElementById("lblPlaceSpotMenu"),
  lblPlaceSpotTips: document.getElementById("lblPlaceSpotTips"),
  placeSpotMenu: document.getElementById("placeSpotMenu"),
  placeSpotTips: document.getElementById("placeSpotTips"),
  placeTimeCategoryRow: document.getElementById("placeTimeCategoryRow"),
  placeTimeGroup: document.getElementById("placeTimeGroup"),
  placeCategoryGroup: document.getElementById("placeCategoryGroup"),
  placeMapGroup: document.getElementById("placeMapGroup")
};
...
document.addEventListener("DOMContentLoaded", () => {
  initAppState();
  setupEventListeners();
  updateExchangeRate(); // 앱 시작 시 실시간 환율 정보 업데이트
});
...
function renderExploreView() {
  ...
  if (reminderEl) {
    if (travelData && travelData.startDate) {
      ...
      let text = "";
      const tripTitle = travelData.title || "여행";
      if (diffDays > 0) {
        text = `'${tripTitle}' D-${diffDays} 여행준비는 다 끝나셨나요 ?`;
      } else if (diffDays === 0) {
        text = `'${tripTitle}' D-Day 여행준비는 다 끝나셨나요 ?`;
      } else {
        text = `'${tripTitle}' D+${Math.abs(diffDays)} 여행은 즐거우셨나요 ?`;
      }
  ...
  if (banner) {
    ...
    banner.onclick = () => {
      openExploreGuideModal(cityCode);
    };
  }
  ...
  if (citiesGrid) {
    ...
    exp.cities.forEach(city => {
      ...
      // Navigate to detailed view's spots tab with this city filter on click
      card.addEventListener("click", () => {
        currentView = "detail";
        activeBottomTab = "spots";
        currentCityFilter = city.filter || "all";
        renderApp();
        showToast(`🗺️ '${city.name}' 지역의 추천 스팟을 보여드립니다!`, "success");
      });
...
function ensureFoodListsExist() {
  ...
  if (!travelData.likedSpots) {
    travelData.likedSpots = [];
  }
...
window.setExtraSubTab = function(subTab) {
  ...
  const btnTools = document.getElementById("btnSubTools");
  ...
  const wrapperTools = document.getElementById("wrapperTools");
  ...
  if (wrapperTools) wrapperTools.classList.add("hidden");
  ...
  if (subTab === "checklist") {
    ...
    if (wrapperTools) wrapperTools.classList.remove("hidden");
    renderToolsTab();
  } else if (subTab === "etc") {
...
function renderSpotsList() {
  ...
  mergedList.forEach((item) => {
    ...
    const defaultImage = "https://images.unsplash.com/photo-1493976040374-85c8e12f0c0e?auto=format&fit=crop&w=600&q=80";

    const isLiked = (travelData.likedSpots || []).some(
      liked => liked.city === item.city && liked.originalIndex === item.originalIndex
    );

    card.innerHTML = `
      ${isEditor && item.isCustom === true ? `
        <button class="btn-card-action btn-edit" style="position: absolute; top: 12px; right: 42px; font-size: 1.15rem; z-index: 10;" onclick="openFoodEditModal('${item.city}', ${item.originalIndex}, event)" title="스팟 수정">
          <i class="ri-edit-box-line"></i>
        </button>
        <button class="btn-card-action btn-delete" style="position: absolute; top: 12px; right: 14px; font-size: 1.15rem; z-index: 10;" onclick="deleteFoodItem('${item.city}', ${item.originalIndex}, event)" title="스팟 삭제">
          <i class="ri-delete-bin-line"></i>
        </button>
      ` : ""}
      
      <!-- 컴팩트 요약 영역 (기본 노출, 클릭 시 펼침) -->
      <div class="food-card-compact-header" style="cursor: pointer; padding: 16px; display: flex; align-items: center; justify-content: space-between; gap: 12px;" onclick="toggleFoodCardAccordion(this)">
        <div class="food-compact-text" style="flex: 1; min-width: 0; text-align: left; display: flex; flex-direction: column; gap: 4px;">
          <div style="display: flex; align-items: center; gap: 8px;">
            <h3 class="food-name" style="font-size: 0.95rem; font-weight: 800; margin: 0; color: var(--text-main); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; max-width: calc(100% - 35px);">${escapeHTML(item.name)}</h3>
            <button type="button" class="btn-like-spot ${isLiked ? 'liked' : ''}" onclick="toggleLikeSpot('${item.city}', ${item.originalIndex}, event)" title="${isLiked ? '찜 해제' : '스팟 찜하기'}" style="background: none; border: none; font-size: 1.15rem; padding: 0; cursor: pointer; display: inline-flex; align-items: center; transition: transform 0.2s;">
              <i class="${isLiked ? 'ri-heart-fill' : 'ri-heart-line'}" style="color: ${isLiked ? 'var(--danger)' : 'var(--text-sub)'};"></i>
            </button>
          </div>
          <div class="food-compact-menu" style="font-size: 0.76rem; color: var(--text-sub); white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">
            <i class="ri-thumb-up-fill" style="color: var(--success); font-size: 0.75rem;"></i>
            <span>추천: <strong>${escapeHTML(item.menu)}</strong></span>
          </div>
          <div class="food-compact-meta" style="display: flex; align-items: center; gap: 6px; flex-wrap: wrap;">
            <span class="badge" style="font-size: 0.68rem; padding: 2px 5px; background-color: ${item.city === 'otaru' || item.city === 'kyoto' || item.city === 'asakusa' ? 'var(--secondary)' : 'var(--primary)'}">${cityLabel}</span>
            <span class="badge badge-meal" style="font-size: 0.68rem; padding: 2px 5px;">${escapeHTML(catInfo.label)}</span>
            <span class="food-rating" style="font-size: 0.72rem; display: inline-flex; align-items: center; gap: 2px;"><i class="ri-star-fill" style="color: #f1c40f;"></i> ${item.rating}</span>
          </div>
        </div>
        <!-- 우측 대표 이미지 썸네일 (접혀있을 때 노출) -->
        <div class="food-thumbnail-wrapper" style="width: 64px; height: 64px; border-radius: 8px; overflow: hidden; flex-shrink: 0; border: 1px solid var(--border-glass);">
          <img src="${item.imageUrl || defaultImage}" alt="${escapeHTML(item.name)}" style="width: 100%; height: 100%; object-fit: cover;">
        </div>
        <!-- 우측 토글 화살표 -->
        <i class="ri-arrow-down-s-line accordion-arrow-icon" style="font-size: 1.25rem; color: var(--text-sub); transition: transform 0.3s;"></i>
      </div>
      
      <!-- 상세 정보 및 지도 액션 영역 (평상시 숨김) -->
      <div class="food-details-collapsible" style="max-height: 0; overflow: hidden; transition: max-height 0.3s ease-out; padding: 0 16px;">
        <div style="border-top: 1px dashed rgba(255,255,255,0.06); padding-top: 14px; padding-bottom: 16px; text-align: left; display: flex; flex-direction: column; gap: 12px;">
          
          <!-- 대표 와이드 이미지 배너 (펼쳤을 때 노출) -->
          <div class="food-banner-wrapper" style="width: 100%; height: 130px; border-radius: 10px; overflow: hidden; border: 1px solid var(--border-glass);">
            <img src="${item.imageUrl || defaultImage}" alt="${escapeHTML(item.name)}" style="width: 100%; height: 100%; object-fit: cover;">
          </div>
          ...
          <div style="display: flex; align-items: center; gap: 6px; flex-wrap: wrap;">
            ${item.openTime ? `<span class="food-rating" style="background: rgba(255,255,255,0.06); color: var(--text-sub); display: inline-flex; align-items: center; gap: 4px; padding: 3px 6px; border-radius: 6px; font-size: 0.72rem;" title="영업 시간"><i class="ri-time-line"></i> ${item.openTime} ~ ${item.closeTime}</span>` : ""}
            ${item.breakStart && item.breakEnd ? `<span class="food-rating" style="background: rgba(255,118,117,0.08); color: var(--danger); display: inline-flex; align-items: center; gap: 4px; padding: 3px 6px; border-radius: 6px; font-size: 0.72rem;" title="브레이크 타임"><i class="ri-pause-circle-line"></i> 브레이크: ${item.breakStart} ~ ${item.breakEnd}</span>` : ""}
          </div>
          ` : ""}
          
          <!-- 대표 추천 메뉴 / 상세 설명 -->
          <div class="food-recommend-menu" style="display: block; line-height: 1.5; white-space: normal; overflow: visible; word-break: break-all; font-size: 0.8rem; background: rgba(255,255,255,0.02); border-radius: 8px; padding: 8px 10px; border-left: 3px solid var(--success);">
            <i class="ri-thumb-up-fill" style="color: var(--success); float: left; margin-right: 6px; margin-top: 2px;"></i>
            <span style="display: inline; white-space: normal; overflow: visible;">추천: <strong>${escapeHTML(item.menu)}</strong></span>
          </div>
          
          <!-- 이용 꿀팁 및 상세 메모 -->
          ${item.tips ? `<p class="food-tips" style="font-size: 0.76rem; color: var(--text-sub); margin: 0; line-height: 1.5; white-space: pre-wrap;"><i class="ri-information-line" style="color: var(--secondary); font-size: 0.82rem;"></i> ${escapeHTML(item.tips)}</p>` : ""}
          
          <!-- 구글맵 위치 바로가기 / 길찾기 / 일정 추가 액션 바 -->
          <div class="food-card-footer" style="border-top: 1px dashed rgba(255,255,255,0.06); padding-top: 12px; display: flex; gap: 8px; margin-top: 4px;">
            <a href="${googleMapSearchUrl}" target="_blank" class="btn-map-action btn-map-view" style="flex: 1; text-align: center; justify-content: center; min-width: 60px; font-size: 0.75rem; text-decoration: none;" title="구글맵에서 장소 주소 검색"><i class="ri-map-pin-2-fill"></i> 지도</a>
            <a href="${directionUrl}" target="_blank" class="btn-map-action btn-map-dir" style="flex: 1; text-align: center; justify-content: center; min-width: 60px; font-size: 0.75rem; text-decoration: none;" title="내 위치에서 길찾기"><i class="ri-navigation-fill"></i> 길찾기</a>
            ${isEditor ? `
              <button type="button" class="btn-map-action btn-map-dir" style="flex: 1.4; text-align: center; justify-content: center; min-width: 80px; font-size: 0.75rem; background: rgba(46, 204, 113, 0.08); color: var(--success); border-color: rgba(46, 204, 113, 0.15);" onclick="addSpotToTimeline('${item.city}', ${item.originalIndex})" title="이 장소를 내 여행 일정에 추가"><i class="ri-calendar-todo-line"></i> 일정 추가</button>
            ` : ""}
          </div>
        </div>
      </div>
...
window.deleteFoodItem = function(city, originalIndex, event) {
  if (event) event.stopPropagation(); // 아코디언 토글 전파 방지
  ...
  const item = list ? list[originalIndex] : null;
  if (item && !item.isCustom) {
    showToast("⚠️ 기본 추천 스팟은 삭제할 수 없습니다.", "error");
    return;
  }
...
window.openFoodEditModal = function(city, originalIndex, event) {
  if (event) event.stopPropagation(); // 아코디언 토글 전파 방지
  ...
  if (!item.isCustom) {
    showToast("⚠️ 기본 추천 스팟은 수정할 수 없습니다.", "error");
    return;
  }
  ...
  sortedItems.forEach((item, index) => {
    ...
    itemEl.innerHTML = `
      ...
      <div class="card glass-card timeline-card ${item.completed ? "completed" : ""}">
        ...
        <!-- 추천 스팟 정보 노출 (있을 때만) -->
        ${(item.recommendMenu || item.tips) ? `
        <div class="place-spot-info-box" style="margin: 8px 0 12px 0; background: rgba(46, 204, 113, 0.05); border-left: 3px solid var(--success); padding: 8px 12px; border-radius: 0 8px 8px 0; font-size: 0.8rem; text-align: left; line-height: 1.4;">
          ${item.recommendMenu ? `<div style="color: var(--text-main); margin-bottom: 4px;">✨ <strong>추천</strong>: ${escapeHTML(item.recommendMenu)}</div>` : ""}
          ${item.tips ? `<div style="color: var(--text-sub);">💡 <strong>꿀팁</strong>: ${escapeHTML(item.tips)}</div>` : ""}
        </div>
        ` : ""}
        
        <!-- 사용자 상세 메모 노출 -->
        ${item.memo ? `<p class="place-memo" style="margin-top: 4px; font-weight: 500; font-size: 0.84rem;"><i class="ri-edit-2-line" style="color: var(--primary); font-size: 0.82rem;"></i> ${escapeHTML(item.memo)}</p>` : ""}
...
window.openEditModal = function(dayKey, index) {
  ...
  if (item.recommendMenu || item.tips) {
    if (elements.placeSpotMenu) elements.placeSpotMenu.value = item.recommendMenu || "";
    if (elements.placeSpotTips) elements.placeSpotTips.value = item.tips || "";
    if (elements.lblPlaceSpotMenu) elements.lblPlaceSpotMenu.innerHTML = item.recommendMenu ? `✨ <strong>추천</strong>: ${escapeHTML(item.recommendMenu)}` : "";
    if (elements.lblPlaceSpotTips) elements.lblPlaceSpotTips.innerHTML = item.tips ? `💡 <strong>꿀팁</strong>: ${escapeHTML(item.tips)}` : "";
    if (elements.placeSpotInfoWrapper) elements.placeSpotInfoWrapper.classList.remove("hidden");
    
    elements.placeName.readOnly = true;
    elements.placeCategory.disabled = true;

    // 추천 스팟 필드 최적화 (분류, 주소 감추기 및 시간 그리드 1열 처리)
    if (elements.placeCategoryGroup) elements.placeCategoryGroup.classList.add("hidden");
    if (elements.placeMapGroup) elements.placeMapGroup.classList.add("hidden");
    if (elements.placeTimeCategoryRow) elements.placeTimeCategoryRow.style.gridTemplateColumns = "1fr";
  } else {
    if (elements.placeSpotMenu) elements.placeSpotMenu.value = "";
    if (elements.placeSpotTips) elements.placeSpotTips.value = "";
    if (elements.placeSpotInfoWrapper) elements.placeSpotInfoWrapper.classList.add("hidden");
    
    elements.placeName.readOnly = false;
    elements.placeCategory.disabled = false;

    // 일반 일정 필드 복원 (분류, 주소 보이기 및 시간 그리드 2열 처리)
    if (elements.placeCategoryGroup) elements.placeCategoryGroup.classList.remove("hidden");
    if (elements.placeMapGroup) elements.placeMapGroup.classList.remove("hidden");
    if (elements.placeTimeCategoryRow) elements.placeTimeCategoryRow.style.gridTemplateColumns = "1fr 1fr";
  }
...
function setupEventListeners() {
  ...
  const handleOpenAddModal = () => {
    ...
    // Reset locks (추천 스팟 추가로 인한 락 상태 해제)
    elements.placeName.readOnly = false;
    elements.placeCategory.disabled = false;
  ...
  elements.formPlace.addEventListener("submit", async (e) => {
    ...
    const recommendMenu = elements.placeSpotMenu ? elements.placeSpotMenu.value : "";
    const tips = elements.placeSpotTips ? elements.placeSpotTips.value : "";
    
    const itemData = { name, time, category, cost, currency, mapAddress, memo, lat, lng, recommendMenu, tips };
  ...
  elements.formFood.addEventListener("submit", (e) => {
    ...
    const newFoodItem = { name, category, rating, menu, address, openTime, closeTime, breakStart, breakEnd, tips, isCustom: true };
  ...
  // Trip Title & Subtitle 통합 Edit Modal Event
  const handleOpenHeaderEditModal = () => {
    ...
    const defaultSubtitle = `${conceptText} · ${nights}`;

    let cityName = "삿포로 & 오타루";
    if (travelData.cityCode === "tokyo") cityName = "도쿄 (Tokyo)";
    else if (travelData.cityCode === "osaka") cityName = "오사카 & 교토";

    if (elements.inputHeaderTitle) elements.inputHeaderTitle.value = travelData.title || cityName;
    if (elements.inputHeaderSubtitle) {
      elements.inputHeaderSubtitle.value = travelData.subtitle || "";
      elements.inputHeaderSubtitle.placeholder = defaultSubtitle;
    }

    if (elements.modalHeaderEdit) elements.modalHeaderEdit.classList.remove("hidden");
  };

  const handleCloseHeaderEditModal = () => {
    if (elements.modalHeaderEdit) elements.modalHeaderEdit.classList.add("hidden");
  };

  if (elements.btnEditTripTitleDirect) {
    elements.btnEditTripTitleDirect.addEventListener("click", handleOpenHeaderEditModal);
  }
  if (elements.btnHeaderEditClose) {
    elements.btnHeaderEditClose.addEventListener("click", handleCloseHeaderEditModal);
  }
  if (elements.btnHeaderEditCancel) {
    elements.btnHeaderEditCancel.addEventListener("click", handleCloseHeaderEditModal);
  }
  if (elements.formHeaderEdit) {
    elements.formHeaderEdit.addEventListener("submit", (e) => {
      e.preventDefault();
      const newTitle = elements.inputHeaderTitle.value.trim();
      const newSubtitle = elements.inputHeaderSubtitle.value.trim();
      
      if (newTitle) {
        travelData.title = newTitle;
        travelData.subtitle = newSubtitle || "";
        
        saveToLocalStorage();
        renderApp();
        renderHomeTripsGrid(); // 대시보드 카드 갱신
        handleCloseHeaderEditModal();
        showToast("✏️ 여행 정보가 변경되었습니다!", "success");
      }
    });
  }

  // --- Translator Event Listeners ---
  if (elements.btnSwapLang) {
    elements.btnSwapLang.addEventListener("click", () => {
      const temp = translatorSourceLang;
      translatorSourceLang = translatorTargetLang;
      translatorTargetLang = temp;
      
      if (elements.lblSourceLang) elements.lblSourceLang.innerText = translatorSourceLang === "ko" ? "한국어" : "일본어";
      if (elements.lblTargetLang) elements.lblTargetLang.innerText = translatorTargetLang === "ko" ? "한국어" : "일본어";
      
      const srcText = elements.txtSourceText ? elements.txtSourceText.value : "";
      const targetText = elements.txtTargetText ? elements.txtTargetText.innerText : "";
      
      if (elements.txtSourceText && targetText && targetText !== "번역 중 오류가 발생했습니다. 다시 시도해 주세요.") {
        elements.txtSourceText.value = targetText;
        if (elements.txtSourceCharCount) elements.txtSourceCharCount.innerText = `${targetText.length}/500`;
        translateAndDisplay();
      }
    });
  }

  if (elements.txtSourceText && elements.txtSourceCharCount) {
    elements.txtSourceText.addEventListener("input", () => {
      const len = elements.txtSourceText.value.length;
      elements.txtSourceCharCount.innerText = `${len}/500`;
    });
  }

  if (elements.btnClearTranslate) {
    elements.btnClearTranslate.addEventListener("click", () => {
      if (elements.txtSourceText) {
        elements.txtSourceText.value = "";
        if (elements.txtSourceCharCount) elements.txtSourceCharCount.innerText = "0/500";
      }
      if (elements.txtTargetText) elements.txtTargetText.innerText = "";
      if (elements.boxTranslateResult) elements.boxTranslateResult.style.display = "none";
    });
  }

  if (elements.btnSubmitTranslate) {
    elements.btnSubmitTranslate.addEventListener("click", translateAndDisplay);
  }

  if (elements.btnCopyTranslate && elements.txtTargetText) {
    elements.btnCopyTranslate.addEventListener("click", () => {
      const txt = elements.txtTargetText.innerText;
      if (!txt) return;
      
      navigator.clipboard.writeText(txt).then(() => {
        showToast("📋 번역 결과가 클립보드에 복사되었습니다!", "success");
      }).catch(err => {
  ...
  if (elements.btnTriggerOcr && elements.inputOcrImage) {
    elements.btnTriggerOcr.addEventListener("click", () => {
      elements.inputOcrImage.click();
    });
    
    elements.inputOcrImage.addEventListener("change", (e) => {
      const file = e.target.files[0];
      if (file) {
        runOcrTranslation(file);
      }
    });
  }

  // --- Guide Modal Close Listeners ---
  if (elements.btnGuideModalClose) {
    elements.btnGuideModalClose.addEventListener("click", () => closeExploreGuideModal());
  }
  if (elements.btnGuideModalCancel) {
    elements.btnGuideModalCancel.addEventListener("click", () => closeExploreGuideModal());
  }
...
// ==========================================
// 1-3. 추천 스팟 좋아요(찜하기) 및 홈 뷰 노출 로직
// ==========================================
window.toggleLikeSpot = function(city, originalIndex, event) {
  if (event) event.stopPropagation(); // 카드 아코디언 토글 전파 방지
  
  if (!travelData.likedSpots) {
    travelData.likedSpots = [];
  }
  
  const index = travelData.likedSpots.findIndex(
    item => item.city === city && item.originalIndex === originalIndex
  );
  
  if (index > -1) {
    travelData.likedSpots.splice(index, 1);
    showToast("❤️ 찜한 스팟 목록에서 제외되었습니다.", "info");
  } else {
    travelData.likedSpots.push({ city, originalIndex });
    showToast("❤️ 이 스팟을 찜했습니다! 일정(홈) 탭에서 즉시 확인하실 수 있습니다.", "success");
  }
  
  saveToLocalStorage();
  renderSpotsList(); // 추천 스팟 탭 재렌더링 (하트 상태 업데이트)
  renderLikedSpots(); // 홈 뷰 찜한 스팟 슬라이더 실시간 업데이트
};

window.renderLikedSpots = function() {
  const sectionEl = elements.likedSpotsSection;
  const sliderEl = elements.likedSpotsSlider;
  if (!sectionEl || !sliderEl) return;
  
  const likedList = travelData.likedSpots || [];
  ...
  likedList.forEach((liked) => {
    let list = [];
    if (liked.city === "otaru") {
      list = travelData.otaruFoodList || [];
    } else if (liked.city === "sapporo") {
      list = travelData.sapporoFoodList || [];
    } else {
      const cityCode = travelData.cityCode || "sapporo";
      if (cityCode === "tokyo") {
        list = travelData.tokyoFoodList || [];
    ...
    const item = list[liked.originalIndex];
    if (!item) return;

    const miniCard = document.createElement("div");
    miniCard.className = "liked-spot-mini-card glass-card";
    
    miniCard.innerHTML = `
      <div class="liked-spot-mini-header">
        <img src="${item.imageUrl || defaultImage}" alt="${escapeHTML(item.name)}" class="liked-spot-mini-thumb">
        <div class="liked-spot-mini-info">
          <h4 class="liked-spot-mini-name">${escapeHTML(item.name)}</h4>
          <span class="liked-spot-mini-menu">추천: ${escapeHTML(item.menu)}</span>
        </div>
      </div>
      <div class="liked-spot-mini-actions">
        <button type="button" class="btn-mini-action btn-add-liked-spot" style="background: rgba(46, 204, 113, 0.12); border-color: rgba(46, 204, 113, 0.25);" onclick="addSpotToTimeline('${liked.city}', ${liked.originalIndex})">
          <i class="ri-add-line"></i> 일정추가
        </button>
        <button type="button" class="btn-mini-action btn-dislike-liked-spot" style="flex: 0 0 40px;" onclick="toggleLikeSpot('${liked.city}', ${liked.originalIndex}, event)">
          <i class="ri-heart-fill" style="color: var(--danger);"></i>
        </button>
      </div>
    `;
    sliderEl.appendChild(miniCard);
  });
};

// ==========================================
// 1-4. 실시간 날씨 및 날씨 예보 관련 로직
// ==========================================
// 도시별 계절 기후 정보 및 추천 옷차림 가이드 데이터
const CITY_SEASON_WEATHER = {
  sapporo: {
...
// OpenWeather API 기반 실시간 날씨 데이터 가져오기
async function getWeatherData(cityCode) {
  const OPENWEATHER_API_KEY = ""; // 임시 더미 날씨로 동작
  ...
  try {
    ...
    console.error("Weather fetch failed, falling back to dummy:", err);
    let dummy = { temp: 18, desc: "구름 조금 (더미)", icon: "☁️" };
    if (cityCode === "tokyo") dummy = { temp: 22, desc: "맑음 (더미)", icon: "☀️" };
    else if (cityCode === "osaka") dummy = { temp: 24, desc: "소나기 (더미)", icon: "🌦️" };
    return dummy;
  }
}

// OpenWeather API 기반 5일 예보 데이터 가져오기
async function getForecastData(cityCode) {
  const OPENWEATHER_API_KEY = ""; // 임시 더미 날씨로 동작
  ...
  try {
    ...
    // 12:00 기준 데이터 추출
    const list = data.list.filter(item => item.dt_txt.includes("12:00:00")).slice(0, 4);
...
async function updateWeatherCard(cityCode) {
  try {
    const data = await getWeatherData(cityCode);
    if (elements.weatherIcon) elements.weatherIcon.innerText = data.icon;
    if (elements.txtWeatherValue) elements.txtWeatherValue.innerText = `${data.temp}°C`;
    if (elements.weatherLabel) elements.weatherLabel.innerText = data.desc;

    // 5일 날씨 예보 바인딩
    const forecast = await getForecastData(cityCode);
    if (elements.forecastContainer) {
      ...
      forecast.forEach(f => {
        ...
        card.innerHTML = `
          <span style="font-size: 0.72rem; font-weight: 700; color: var(--text-sub);">${f.day}</span>
          <span style="font-size: 1.6rem; margin: 4px 0;">${f.icon}</span>
          <span style="font-size: 0.84rem; font-weight: 800; color: var(--text-main);">${f.temp}°C</span>
          <span style="font-size: 0.7rem; color: var(--text-sub);">${f.desc}</span>
        `;
    ...
    const seasonData = CITY_SEASON_WEATHER[cityCode]?.[season] || CITY_SEASON_WEATHER.sapporo.summer;
    if (elements.weatherGuideText) {
      elements.weatherGuideText.innerHTML = `현재 ${currentMonth}월 계절: <strong>${season === 'spring' ? '봄 🌸' : season === 'summer' ? '여름 ☀️' : season === 'autumn' ? '가을 🍁' : '겨울 ❄️'}</strong> (${seasonData.desc})<br>${seasonData.tips}`;
    }
  } catch (err) {
    console.error("Failed to update weather UI:", err);
  }
}

window.renderToolsTab = function() {
  const cityCode = travelData.cityCode || "sapporo";
  
  // 1. 실시간 날씨 데이터 카드 갱신
  updateWeatherCard(cityCode);
  
  // 1-2. 실시간 환율 정보 카드 갱신
  updateExchangeRate();
  
  // 1-3. 번역기 기본 언어 텍스트 리셋
  if (elements.lblSourceLang) elements.lblSourceLang.innerText = translatorSourceLang === "ko" ? "한국어" : "일본어";
  if (elements.lblTargetLang) elements.lblTargetLang.innerText = translatorTargetLang === "ko" ? "한국어" : "일본어";
};
...
async function runOcrTranslation(file) {
  if (!file) return;
  
  if (elements.boxOcrStatus) {
    elements.boxOcrStatus.style.display = "flex";
    if (elements.txtOcrStatusMsg) elements.txtOcrStatusMsg.innerText = "OCR 엔진 로딩 중...";
  }
  if (elements.boxTranslateResult) elements.boxTranslateResult.style.display = "none";
  ...
  try {
    ...
    if (elements.txtOcrStatusMsg) elements.txtOcrStatusMsg.innerText = "이미지 분석 및 글자 추출 중...";
    
    const lang = translatorSourceLang === "ko" ? "kor" : "jpn";
    const result = await Tesseract.recognize(file, lang);
    const extractedText = result.data.text || "";
    
    if (elements.txtSourceText) {
      elements.txtSourceText.value = extractedText;
      if (elements.txtSourceCharCount) elements.txtSourceCharCount.innerText = `${extractedText.length}/500`;
    }
    
    if (extractedText.trim() === "") {
      showToast("이미지에서 글자를 발견하지 못했습니다.", "error");
      if (elements.boxOcrStatus) elements.boxOcrStatus.style.display = "none";
      return;
    }

    if (elements.txtOcrStatusMsg) elements.txtOcrStatusMsg.innerText = "추출된 글자 번역 중...";
    
    const translatedText = await translateText(extractedText, translatorSourceLang, translatorTargetLang);
    
    if (elements.txtTargetText) elements.txtTargetText.innerText = translatedText;
    if (elements.boxTranslateResult) elements.boxTranslateResult.style.display = "flex";
    if (elements.boxOcrStatus) elements.boxOcrStatus.style.display = "none";
    
    showToast("🎉 사진 번역이 성공적으로 완료되었습니다!", "success");
  } catch (err) {
    console.error("OCR Translation failed:", err);
    showToast("사진 인식에 실패했습니다.", "error");
    if (elements.boxOcrStatus) elements.boxOcrStatus.style.display = "none";
  }
}

async function translateAndDisplay() {
  if (!elements.txtSourceText || !elements.txtTargetText) return;
  const text = elements.txtSourceText.value;
  if (text.trim() === "") {
    elements.txtTargetText.innerText = "";
    if (elements.boxTranslateResult) elements.boxTranslateResult.style.display = "none";
    return;
  }
  
  if (elements.txtTargetText) {
    elements.txtTargetText.innerText = "번역 중...";
    if (elements.boxTranslateResult) elements.boxTranslateResult.style.display = "flex";
  }
  
  const result = await translateText(text, translatorSourceLang, translatorTargetLang);
  if (elements.txtTargetText) elements.txtTargetText.innerText = result;
}

// ==========================================
// 1-6. 실시간 환율 정보 캐싱 및 업데이트 로직
// ==========================================
async function updateExchangeRate() {
  const todayStr = new Date().toISOString().split("T")[0]; // YYYY-MM-DD
  
  try {
    const cachedRate = localStorage.getItem("sapo_exchange_rate");
    const cachedDate = localStorage.getItem("sapo_exchange_rate_date");
    const cachedTime = localStorage.getItem("sapo_exchange_rate_time");
    
    if (cachedRate && cachedDate === todayStr) {
      const rateVal = parseFloat(cachedRate);
      if (!isNaN(rateVal)) {
        CURRENCY_CONVERSION_RATE = rateVal;
        
        const rateText = `₩ ${Math.round(rateVal * 100 * 100) / 100}원`;
        if (elements.txtExchangeRateValue) elements.txtExchangeRateValue.innerText = rateText;
        if (elements.txtExchangeRateTime && cachedTime) elements.txtExchangeRateTime.innerText = `(${cachedTime} 업데이트)`;
        
        updateDashboardStats();
        if (activeBottomTab === "extra" && activeExtraSubTab === "settlement") {
          renderSettlementTab();
        }
        console.log(`[Exchange Rate Cached] Loaded today (${todayStr}): 1 JPY = ${CURRENCY_CONVERSION_RATE} KRW`);
        return;
      }
    }
  } catch (e) {
    console.warn("Exchange cache read failed:", e);
  }
  ...
  if (rateOneJPY && !isNaN(rateOneJPY)) {
    CURRENCY_CONVERSION_RATE = rateOneJPY;
    const rateText = `₩ ${Math.round(rateOneJPY * 100 * 100) / 100}원`;
    if (elements.txtExchangeRateValue) elements.txtExchangeRateValue.innerText = rateText;
    ...
    if (elements.txtExchangeRateTime) elements.txtExchangeRateTime.innerText = `(${timeStr} 업데이트)`;

    try {
      localStorage.setItem("sapo_exchange_rate", rateOneJPY.toString());
      localStorage.setItem("sapo_exchange_rate_date", todayStr);
      localStorage.setItem("sapo_exchange_rate_time", timeStr);
    } catch (e) {
    ...
    console.log(`[Exchange Rate Synced] 1 JPY = ${CURRENCY_CONVERSION_RATE} KRW`);
  } else {
    CURRENCY_CONVERSION_RATE = 9.0;
    if (elements.txtExchangeRateValue) elements.txtExchangeRateValue.innerText = "₩ 900.00원 (대체 환율)";
  }
}

// ==========================================
// 1-7. HOT 추천 가이드북 시각화 리뉴얼 파서
// ==========================================
function parseInlineStyles(text) {
  ...
  // 2. `배지` -> <span class="badge" style="font-size: 0.72rem; padding: 2px 6px; background: rgba(26,115,232,0.1); color: var(--primary); border: 1px solid rgba(26,115,232,0.2); border-radius: 4px; font-weight: 700; margin: 0 2px;">$1</span>
  html = html.replace(/`(.*?)`/g, '<span class="badge" style="font-size: 0.72rem; padding: 2px 6px; background: rgba(26,115,232,0.1); color: var(--primary); border: 1px solid rgba(26,115,232,0.2); border-radius: 4px; font-weight: 700; margin: 0 2px;">$1</span>');
  
  // 3. '배지' -> <span class="badge" style="font-size: 0.72rem; padding: 2px 6px; background: rgba(108,92,231,0.1); color: var(--secondary); border: 1px solid rgba(108,92,231,0.2); border-radius: 4px; font-weight: 700; margin: 0 2px;">$1</span>
  html = html.replace(/'(.*?)'/g, '<span class="badge" style="font-size: 0.72rem; padding: 2px 6px; background: rgba(108,92,231,0.1); color: var(--secondary); border: 1px solid rgba(108,92,231,0.2); border-radius: 4px; font-weight: 700; margin: 0 2px;">$1</span>');
  
  return html;
}

window.parseGuidebookContent = function(content) {
  if (!content) return "";
  
  // 줄바꿈이 이스케이프 텍스트 형태로 들어와도 실제 개행문자로 갱신
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.startsWith("[VS]")) {
      ...
      const vsContent = line.substring(4).trim();
      ...
      if (parts.length >= 2) {
        ...
        htmlResult += `
          <div class="vs-compare-container" style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin: 16px 0;">
            <div class="vs-card vs-left glass-card" style="background: rgba(26, 115, 232, 0.04); border: 1px solid rgba(26, 115, 232, 0.15); border-radius: 12px; padding: 16px 12px; font-size: 0.8rem; text-align: left; position: relative;">
              <span style="position: absolute; top: -8px; left: 10px; font-size: 0.65rem; background: var(--primary); color: white; padding: 2px 6px; border-radius: 6px; font-weight: 800;">OPTION A</span>
              <div style="margin-top: 4px; line-height: 1.5; font-weight: 500;">${parseInlineStyles(left)}</div>
            </div>
            <div class="vs-card vs-right glass-card" style="background: rgba(108, 92, 231, 0.04); border: 1px solid rgba(108, 92, 231, 0.15); border-radius: 12px; padding: 16px 12px; font-size: 0.8rem; text-align: left; position: relative;">
              <span style="position: absolute; top: -8px; left: 10px; font-size: 0.65rem; background: var(--secondary); color: white; padding: 2px 6px; border-radius: 6px; font-weight: 800;">OPTION B</span>
              <div style="margin-top: 4px; line-height: 1.5; font-weight: 500;">${parseInlineStyles(right)}</div>
            </div>
          </div>
        `;
      } else {
        htmlResult += `
          <div style="display: flex; align-items: center; justify-content: center; gap: 8px; margin: 20px 0 10px 0; font-weight: 800; font-size: 0.9rem; color: var(--text-main);">
            <span style="height: 1px; background: rgba(255,255,255,0.08); flex: 1;"></span>
            <span style="display: inline-flex; align-items: center; gap: 4px;">⚖️ ${parseInlineStyles(vsContent)}</span>
            <span style="height: 1px; background: rgba(255,255,255,0.08); flex: 1;"></span>
          </div>
        `;
      }
    ...
    if (line.startsWith("-")) {
      if (!listOpen) {
        htmlResult += `<ul style="list-style: none; padding-left: 0; margin: 10px 0; display: flex; flex-direction: column; gap: 8px; text-align: left;">`;
        listOpen = true;
      }
      const listContent = line.substring(1).trim();
      htmlResult += `
        <li style="display: flex; align-items: flex-start; gap: 8px; font-size: 0.8rem; line-height: 1.5; color: var(--text-sub);">
          <i class="ri-checkbox-circle-fill" style="color: var(--success); font-size: 0.95rem; margin-top: 1px; flex-shrink: 0;"></i>
          <span style="font-weight: 500;">${parseInlineStyles(listContent)}</span>
        </li>
      `;
      continue;
    }
    ...
    // 3. 일반 단락
    htmlResult += `<p style="text-align: left; font-size: 0.82rem; line-height: 1.6; color: var(--text-sub); margin: 8px 0 12px 0; font-weight: 500;">${parseInlineStyles(line)}</p>`;
  }
...
window.openExploreGuideModal = function(cityCode) {
  const modal = elements.modalExploreGuide;
  const titleEl = document.getElementById("guideModalTitle");
  const bodyEl = elements.guideModalBody;
  
  if (!modal || !bodyEl) return;
  
  const temp = CITY_TEMPLATES[cityCode] || CITY_TEMPLATES.sapporo;
  const exp = temp.explore || {};
  const guidebook = exp.guidebook || [];
  
  if (titleEl) {
    let cityName = "삿포로 & 오타루";
    if (cityCode === "tokyo") cityName = "도쿄";
    else if (cityCode === "osaka") cityName = "오사카 & 교토";
    titleEl.innerText = `💡 ${cityName} 추천 가이드`;
  }
  
  bodyEl.innerHTML = "";
  
  if (guidebook.length === 0) {
    bodyEl.innerHTML = `<div style="text-align: center; color: var(--text-sub); padding: 20px;">가이드북 콘텐츠가 비어 있습니다.</div>`;
  } else {
    guidebook.forEach((item, index) => {
      const accItem = document.createElement("div");
      accItem.className = "guide-accordion-item";
      
      const parsedHTML = window.parseGuidebookContent(item.content);
      
      accItem.innerHTML = `
        <div class="guide-accordion-header" onclick="toggleGuideAccordion(this)">
          <span style="display: inline-flex; align-items: center; gap: 8px;">
            <span style="font-size: 1.1rem;">${item.emoji || '💡'}</span>
            <span>${escapeHTML(item.title)}</span>
          </span>
          <i class="ri-arrow-down-s-line guide-accordion-icon"></i>
        </div>
        <div class="guide-accordion-content">
          ${parsedHTML}
        </div>
      `;
      bodyEl.appendChild(accItem);
    });
  }
...
window.closeExploreGuideModal = function() {
  const modal = elements.modalExploreGuide;
  if (modal) {
...
window.toggleGuideAccordion = function(headerEl) {
  const itemEl = headerEl.closest(".guide-accordion-item");
  if (!itemEl) return;
  
  const isActive = itemEl.classList.contains("active");
  
  const allItems = elements.guideModalBody.querySelectorAll(".guide-accordion-item");
  ...
  if (!isActive) {
    ...
    const content = itemEl.querySelector(".guide-accordion-content");
    if (content) {
      content.style.maxHeight = content.scrollHeight + "px";
      
      // Clear maxHeight constraint after transition completes to support resizing and prevent truncation
      setTimeout(() => {
...
window.addSpotToTimeline = function(city, originalIndex) {
  if (!isEditor) {
    showToast("⚠️ 편집자 권한이 필요합니다.", "error");
    return;
  }
  ...
  // 방문지 일정 등록 모달 초기화
  elements.modalTitle.innerText = "추천 장소 일정 추가";
  elements.editItemIndex.value = ""; // 신규 추가 모드
  elements.formPlace.reset();
  ...
  elements.placeCost.value = "";
  elements.placeCurrency.value = "JPY";
  elements.placeMap.value = item.address || "";
  
  // 추천 스팟 정보 숨김/보임 레이아웃 및 락 처리
  if (elements.placeSpotMenu) elements.placeSpotMenu.value = item.menu || "";
  if (elements.placeSpotTips) elements.placeSpotTips.value = item.tips || "";
  if (elements.lblPlaceSpotMenu) elements.lblPlaceSpotMenu.innerHTML = item.menu ? `✨ <strong>추천</strong>: ${escapeHTML(item.menu)}` : "";
  if (elements.lblPlaceSpotTips) elements.lblPlaceSpotTips.innerHTML = item.tips ? `💡 <strong>꿀팁</strong>: ${escapeHTML(item.tips)}` : "";
  if (elements.placeSpotInfoWrapper) elements.placeSpotInfoWrapper.classList.remove("hidden");
  
  // 나의 상세 메모는 비움
  elements.placeMemo.value = "";
  
  // 추천 스팟 폼 필드 제어 (분류, 주소 감추기 및 시간 그리드 1열 처리)
  if (elements.placeCategoryGroup) elements.placeCategoryGroup.classList.add("hidden");
  if (elements.placeMapGroup) elements.placeMapGroup.classList.add("hidden");
  if (elements.placeTimeCategoryRow) elements.placeTimeCategoryRow.style.gridTemplateColumns = "1fr";
...
window.toggleFoodCardAccordion = function(headerEl) {
  const cardEl = headerEl.closest(".food-card");
  if (!cardEl) return;
  
  const collapsible = cardEl.querySelector(".food-details-collapsible");
  const arrow = cardEl.querySelector(".accordion-arrow-icon");
